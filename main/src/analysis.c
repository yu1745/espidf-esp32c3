...
void analysis_timer_callback(TimerHandle_t xTimer) {
  ...
  if (idle_task_handle != NULL) {
    ...
    uint32_t idle_ticks_diff = task_status.ulRunTimeCounter - last_idle_ticks;
    ...
    if (time_diff > 0) {
      ...
      // 发送CPU占用事件
      esp_err_t ret =
          esp_event_post(ANALYSIS_EVENT, ANALYSIS_EVENT_CPU_USAGE, &event_data,
...
float get_cpu_usage(void) { return cpu_usage_percent; }
