...
std::string getBuildParameters() {
    ...
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_WIFI":%d)", CONFIG_ENABLE_WIFI);
#endif
#ifdef CONFIG_ENABLE_BLE
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_BLE":%d)", CONFIG_ENABLE_BLE);
#endif
#ifdef CONFIG_ENABLE_LED
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_LED":%d)", CONFIG_ENABLE_LED);
#endif
#ifdef CONFIG_ENABLE_TEMP
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_TEMP":%d)", CONFIG_ENABLE_TEMP);
#endif
#ifdef CONFIG_ENABLE_BUTTON
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_BUTTON":%d)", CONFIG_ENABLE_BUTTON);
#endif
#ifdef CONFIG_ENABLE_VOLTAGE
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_VOLTAGE":%d)", CONFIG_ENABLE_VOLTAGE);
#endif
#ifdef CONFIG_ENABLE_DECOY
    n += snprintf(json_buffer.get() + n, 1024 - n,
                  R"(, "ENABLE_DECOY":%d)", CONFIG_ENABLE_DECOY);
#endif
