...
// RMT分辨率，与LED灯带使用相同的频率（10MHz）
#define RMT_ACTUATOR_RES_HZ (10 * 1000 * 1000)

namespace actuator {

RMTActuator::RMTActuator(int gpio_num, float offset)
    : Actuator(offset), m_gpio_num(gpio_num) {
  // 初始化RMT
  if (!initRMT()) {
...
bool RMTActuator::initRMT() {
  ...
  rmt_tx_channel_config_t tx_channel_config = {
      ...
      .resolution_hz = 1000 * 1000,                    // 分辨率，1MHz
      .mem_block_symbols = 48, // 内存块大小，48 * 4 = 192字节
      .trans_queue_depth = 4,  // 传输队列深度
      .intr_priority = 0,      // 中断优先级，0表示使用低优先级中断
      .flags = {
          .invert_out = 0,   // 不反转输出
          .with_dma = 0,     // 不使用DMA
  ...
  rmt_simple_encoder_config_t encoder_config = {
      ...
      .min_chunk_size = 1         // 最小块大小，至少需要1个符号(高电平和低电平)
  };
...
bool RMTActuator::actuate(int wait) {
  ...
  // 等待传输完成
  if (wait > 0) {
    ret = rmt_tx_wait_all_done(m_tx_channel, pdMS_TO_TICKS(wait));
    if (ret != ESP_OK) {
...
size_t RMTActuator::rmt_encoder_cb(const void *data, size_t data_size,
  ...
  // 我们需要1个符号
  if (symbols_free < 1) {
    ESP_LOGD(TAG, "Not enough symbol space: %d, required: 1", symbols_free);
    return 0; // 返回0表示没有写入任何符号
  }

  // 计算高电平和低电平的持续时间（以RMT tick为单位）
  // RMT分辨率是10MHz，所以1微秒 = 10 ticks
  // 使用 uint64_t 避免溢出：pulse_width * 10,000,000 / 1,000,000 = pulse_width
  // * 10
  uint16_t high_ticks = pulse_width;
  uint16_t low_ticks = 0; // 低电平固定500us
  // uint16_t high_ticks = 10000;
  // uint16_t low_ticks = 20000;
  // 合成uint32_t值后一次性赋值
  // 位域布局: duration0(15bits) | level0(1bit) | duration1(15bits) |
  // level1(1bit)
  symbols[symbols_written].val =
      (high_ticks & 0x7FFF) | // duration0: bits 0-14
      (1 << 15) |             // level0: bit 15 = 1 (高电平)
      ((low_ticks & 0x7FFF)
       << 16); // duration1: bits 16-30, level1: bit 31 = 0 (低电平)
  // static int t = 0;
  // if (t++ % 50 == 0) {
  //   ESP_LOGI(TAG, "Encoded pulse width %u us to %hu high ticks and %hu low
  //   ticks",
  //            pulse_width, symbols[0].duration0, symbols[0].duration1);
  // }

  // 标记编码完成
  *done = true;

  // ESP_LOGD(TAG, "Encoded pulse width %u us to %u high ticks and %u low
  // ticks",
...
void testRMTActuator(int gpio_num) {
  ...
  try {
    ...
    RMTActuator *rmt_actuator = new RMTActuator(gpio_num);
